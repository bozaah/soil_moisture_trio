...
*Research date: 12 August 2026; artifact-scope decision updated 14 August 2026. The pinned source manifest was authenticated and live-verified on 12 August. Product metadata may change and must be checked against the tracked manifest before a build. SLGA layers are modelled digital-soil-mapping predictions with quantified uncertainty and must never be equated with observed field measurements.*
...
- **Recommended target-cell aggregation:** build the initial artifact for the approved SWAZ buffered operational rectangle on canonical AWRA-L cells derived from authoritative source coordinates, using area-weighted aggregation in Australian Albers (EPSG:3577). SWAZ operational runs select exact coordinate subsets; they do not re-harmonise SLGA. Report source coverage and mapped dispersion per full target-cell footprint. A full-WA static artifact is not planned for this phase.
...
- **Target grid:** the explicit builder derives the SWAZ target grid from the completed 2025 Bureau-produced AWRA-L v7 operational decile file at NCI, pinned by exact URL/path, byte size, independent SHA-256, schema, and coordinate hashes in `manifests/awral_v7_grid_source_v1.json`. The approved initial footprint is the exact contiguous 156×186 subset with descending latitude centres −27.45 through −35.20 and ascending longitude centres 114.05 through 123.30 (29,016 cells). It covers the approved SWAZ boundary’s +0.1° operational bbox; the polygon is applied later as a runtime mask and does not alter static full-cell denominators. The builder validates monotonicity, spacing and cell-centre interpretation and persists source coordinate values without rounding or regeneration. The remote annual object is not assumed immutable; only an explicit local byte copy matching the contract is accepted. SWAZ operational runs must match and subset the persisted coordinates exactly.
...
- **Spatial target grid:** persist the approved SWAZ subset of the canonical AWRA-L grid derived from authoritative source coordinates; require exact coordinate matching when SWAZ operational runs subset the artifact.
...
6. **Deterministic spatial alignment:** canonical SWAZ target coordinates, derived cell edges, full-cell footprints, and source overlaps are checked and reproducible; operational subsets must match persisted coordinates exactly.
...
## 14. Minimal-module specification and implementation status

The narrow prototype is implemented under `src/soil_moisture_trio/slga/` and governed by `docs/slga-builder-contract.md`. It remains enabling work for review: no approved production artifact, operational soil summary, coverage threshold, or soil band has been created.

**(a) Approved product catalogue** — `slga.catalogue` loads the tracked 18-layer manifest and validates immutable full product IDs, property, depth, units, component, version, DOI, licence/citation, native profile, nodata contract, URLs, published STAC multihashes, and known upstream exceptions. Full IDs—not property codes—are catalogue keys; shorthand lookup and silent source substitution are rejected.

**(b) COG retrieval adapter** — `slga.cog` reads bounded full-resolution windows, obtains `TERN_API_KEY` only from the environment, and scopes HTTP Basic/GDAL credentials without persisting or logging them. It applies bounded retries, validates STAC/COG identity and profile, preserves nodata, records retrieval provenance, and uses a bounded block-aligned in-memory LRU cache in the tiled builder path. Instrumentation now separates STAC and COG attempts/outcomes, attempted/completed COG window reads, cache hits/misses/evictions/current/peak bytes, per-tile elapsed time, and process peak RSS. Rasterio/GDAL transferred-byte counts remain unavailable through the reader and are not estimated. A larger representative live profiling run remains incomplete.

**(c) Integration and harmonisation** — `slga.integration`, `slga.harmonise`, and `slga.tiling` implement native-grid DES-capped depth integration and EPSG:3577 fractional-overlap aggregation onto exact canonical AWRA-L cells. Outputs retain valid source area, full-cell area, variable-specific coverage, mapped dispersion, and nodata for zero overlap. Synthetic tests prove exact equality across tested tile shapes and processing orders; this is not yet a SWAZ-wide performance or completeness result.

**(d) Artifact and runtime contract** — `slga.artifact` implements the reviewed v1-candidate compressed NetCDF schema, deterministic sidecar, immutable fail-if-present staged bundle publication, checksums, provenance, and a credential-free loader that accepts only exact contiguous coordinate subsets. Direct DES EV/10/90 context, shallow-profile fractions, mixed-width support, and machine-readable case definitions are included. The writer and loader are tested with synthetic artifacts. No production SWAZ NetCDF has been built or promoted, no full-WA static artifact is planned for this phase, and normal drought runs do not load soil context yet.

**(e) Verification** — deterministic tests cover source and manifest drift, authentication failures, nodata, malformed values, uncertainty ordering, CRS/grid mismatch, partial and zero overlap, canonical-grid validation, artifact corruption/schema/checksum failures, exact runtime subsets, interrupted writes, bounded cache behavior, and tile-shape/order invariance. The authenticated live AWC probe remains opt-in. A live 3×3 inland SWAZ multi-tile pilot and 5×5 Albany coastal/nodata single- and nine-tile pilots exercised all 18 sources and seven storage cases. Albany coverage ranged from 0 to 1 (mean 0.6477), with finite storage in 24/25 cells. The nine-tile run matched the single-tile summaries and was exactly equal in reverse order; its warm pass used only cache hits. These pilots establish bounded seam/order behavior, not SWAZ-wide fitness or approved operational integration.

**Dependencies:** Rasterio and pyproj are declared direct dependencies alongside the existing direct NumPy and Shapely dependencies. GDAL remains transitive through Rasterio because project code does not call it directly.
...
2. ~~**Prototype AWC + DES**~~ **Completed for small windows:** the prototype retrieves AWC EV/05/95 and DES EV/10/90 and computes DES-capped modelled 0–100 cm AWC storage capacity, separate component effects, and provisional mixed-quantile scenarios.
3. **Harmonisation prototype completed; scaling review remains:** the canonical AWRA-L v7 grid source and 156×186 SWAZ footprint are pinned; EPSG:3577 fractional-overlap harmonisation, coverage, mapped dispersion, exact runtime subsets, and bounded tiling are implemented. The 3×3 authenticated pilot proved tested multi-tile order invariance. The 5×5 Albany single-tile pilot confirmed expected coastal zero/partial coverage and cache-only repeatability. Its nine-tile follow-up confirmed reverse-order equality with no eviction under a 256 MiB cache, but 2×2 tiling increased logical reads from 18 to 162. A controlled 100-cell comparison then found one 10×10 tile slightly faster than four 5×5 tiles (56.09 versus 57.95 seconds), with 18 versus 72 fetches and 81 versus 144 MiB peak cache, at the cost of ~532 versus ~402 MiB peak RSS. Both had exact warm repeats and identical summaries. Adopt 10×10 tiles and 256 MiB cache provisionally for SWAZ build planning; next review whole-build failure/restart limits and decide whether external HTTP byte measurement is necessary.
4. **Artifact mechanism and reviewed v1 candidate schema completed; production build remains:** the compressed NetCDF writer, deterministic sidecar, credential-free verifying loader, and immutable fail-if-present staged bundle publication are implemented and tested with synthetic artifacts. The user-approved schema adds direct mapped DES EV/10/90 means/dispersion/support, valid-DES-area shallow-than-1 m fractions, mixed-width-specific support/dispersion, and machine-readable case definitions. A clean-commit-only explicit SWAZ command now implements checksum-bound resumable 10-row stripes, six-hour between-stripe limits, exact assembly, audited build-report publication, and cleanup only after immutable bundle verification. Complete external soil-science and distribution/review-authority approval, then explicitly authorise execution under `data/processed/slga_awral/`. Do not build a full-WA static artifact in this phase.
5. **Grouped-summary reconciliation remains:** implement B14a/B25c only after the artifact and scientific grouping rules are approved. Prove soil strata plus uncovered cells reconcile to the valid-risk domain and numerical risk outputs/valid mask remain unchanged.
...
---
*Prepared as a scientific decision-support brief and updated after authenticated source verification and the bounded B25b prototype. The catalogue, retrieval, integration, harmonisation, tiling, provisional artifact I/O, and exact-subset loading foundations are implemented, but no approved SWAZ artifact or operational soil summary exists. A full-WA static artifact is not planned for this phase. Existing risk thresholds, weights, calculations, and valid mask remain unchanged; soil properties are interpretive strata only, not new risk predictors. Any production build must revalidate live source identity against the tracked pinned manifest.*
