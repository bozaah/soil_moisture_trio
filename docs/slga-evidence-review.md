...
3. **Harmonisation prototype completed; scaling review remains:** the canonical AWRA-L v7 grid source and 441×341 WA footprint are pinned; EPSG:3577 fractional-overlap harmonisation, coverage, mapped dispersion, exact runtime subsets, and bounded tiling are implemented. The 3×3 authenticated pilot proved tested tile-order invariance, not full-WA completeness or performance. Next, profile a larger representative coastal/nodata area with measured transfer, retry, cache, memory, and runtime behavior.
4. **Artifact mechanism completed; production build remains:** the provisional compressed NetCDF writer, deterministic sidecar, and credential-free verifying loader are implemented and tested with synthetic artifacts. Review the final schema and DES presentation, decide distribution/version promotion and rollback, provide an explicit reviewed production build command, and only then create the full-WA artifact under `data/processed/slga_awral/`.
