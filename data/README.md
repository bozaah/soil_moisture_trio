...
The reviewed v1 candidate schema includes direct DES EV/10/90 context, shallow-than-1 m fractions, mixed-width support and machine-readable case definitions. Immutable bundle publication fails if the destination exists and preserves prior bundles for rollback. The sidecar records the build-report checksum. The explicit builder implements credential-free checksum-bound 10-row checkpoints under `.slga_awc_des_awral_swaz_0p05deg_v1.checkpoints/`, removing them only after verified publication unless asked to retain them.

Bounded Albany profiling supports 10×10 target tiles with a 256 MiB cache, not a whole-SWAZ performance guarantee. The command and restart mechanism exist. Rodrigo's [2026-08-31 waiver](../sessions/2026-08-31-swaz-review-build-waiver.md) authorises a review-input build only. External review and distribution/release approvals remain outstanding. The SWAZ review bundle was built 2026-09-14 and exists locally under `data/processed/slga_awral/slga_awc_des_awral_swaz_0p05deg_v1/` (3.1 MB). Checkpoints were removed after verified publication.

Git does not protect these local inputs or outputs. The review bundle was copied 2026-09-15 to `/Volumes/Extreme SSD/backups/soil_moisture_trio/data/processed/slga_awral/slga_awc_des_awral_swaz_0p05deg_v1/`; the NetCDF SHA-256 on the drive matches the artifact hash `63b0e8d9…4b266`. One copy, one drive: not a redundant backup. `outputs/` runs, including grouped summaries and HTML pages, are regenerable from the commands in the README and are not backed up.
