...
Large derived files remain excluded from Git. The live-verified source catalogue is tracked at [`../manifests/slga_awc_des_sources_v1.json`](../manifests/slga_awc_des_sources_v1.json); it pins AWC EV/05/95 and DES EV/10/90 full identifiers, URLs, profiles, published STAC multihashes, and source sizes. Reproducibility also requires an explicit build command, output checksums, complete transformation metadata, and a tracked build contract. Operational runs must fail clearly when the approved artifact is absent or incompatible; they must not silently download, rebuild, or substitute a newer SLGA product.
